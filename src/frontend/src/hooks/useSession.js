...
const mergeStreamContent = (current, incoming, replace) => {
    if (!incoming) {
        return current;
    }
...
const isOutputRole = (value) => value === "user" || value === "assistant" || value === "status";
const randomEntryId = () => `restored-${Math.random().toString(36).slice(2)}`;
const sanitizeStoredOutput = (raw) => {
    if (!raw) {
        return [];
    }
    try {
        ...
        return parsed
            ...
            return { id, role, content, pending };
        });
    }
    catch (error) {
...
const inferRoleFromContent = (content) => {
    if (!content) {
        return "assistant";
    }
...
export function useSession() {
    ...
    const persistOutput = useCallback((entries) => {
    ...
    const updateOutput = useCallback((updater) => {
        setOutput((previous) => {
    ...
    const applySessionId = useCallback((value) => {
        ...
        if (value) {
            ...
            const restored = sanitizeStoredOutput(window.localStorage.getItem(`${STORAGE_OUTPUT_PREFIX}${value}`));
            updateOutput(() => restored);
        }
        else {
            window.localStorage.removeItem(STORAGE_KEY);
            if (previousId) {
                window.localStorage.removeItem(`${STORAGE_OUTPUT_PREFIX}${previousId}`);
            }
            updateOutput(() => []);
        }
    }, [updateOutput]);
    ...
    const handleStreamMessage = useCallback((payload, meta = {}) => {
        ...
        const text = payload.text ??
            ...
            payload.message ??
            "";
        const entryIdFromPayload = payload.entry_id ?? payload.id ?? meta.lastEventId ?? undefined;
        const shouldAppend = payload.append ??
        ...
        if (activeEntryIdRef.current) {
            ...
            updateOutput((previous) => previous.map((entry) => {
                if (entry.id !== targetId) {
                    return entry;
                }
                const replaceExisting = payload.replace === true ||
                    !shouldAppend ||
                    entry.pending === true ||
                ...
                return {
                    ...
                    role,
                    content: nextContent,
                    pending: payload.pending ?? (hasText ? false : entry.pending ?? undefined),
                };
            }));
            if (isFinal) {
                setActiveEntryId(null);
        ...
        updateOutput((previous) => {
            ...
            if (index >= 0) {
                ...
                const replaceExisting = payload.replace === true || !shouldAppend || !existing.content;
                const nextContent = hasText
    ...
    const connectStream = useCallback(function connectStreamInner(targetSessionId, options) {
        if (typeof window === "undefined") {
            return;
        }
        if (!("EventSource" in window)) {
            setStreamStatus("unsupported");
            setStreamError("このブラウザは Server-Sent Events をサポートしていません。");
            return;
        }
        ...
        source.onerror = () => {
            ...
            setStreamError("ログストリームとの接続が切断されました。再接続を試みます。");
            if (streamRef.current) {
                streamRef.current.close();
                streamRef.current = null;
            }
    ...
    const sendCommand = useCallback(async (command) => {
        ...
        try {
            ...
            const performRequest = async (targetSessionId) => fetch(`${baseUrl}/${targetSessionId}/input`, {
                method: "POST",
            ...
            if (response.status === 404) {
                updateOutput((previous) => previous.map((entry) => entry.id === pendingEntryId
                    ? {
                        ...entry,
                        role: "status",
    ...
    const cancelCommand = useCallback(async () => {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                const reason = (detail && typeof detail.detail === "string" && detail.detail) ||
